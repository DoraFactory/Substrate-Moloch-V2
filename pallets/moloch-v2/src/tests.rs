...
fn summon_with(initial_member: u64) {
	...
	let _ = MolochV2::summon(Origin::signed(initial_member), period_duration, voting_period_length, grace_period_length, abort_window, dilution_bound, proposal_deposit, processing_reward);
}
...
fn summon_works() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(MolochV2::summon(Origin::signed(1), period_duration, voting_period_length, grace_period_length, abort_window, dilution_bound, proposal_deposit, processing_reward));
		// check the constants
		...
		assert_eq!(MolochV2::abort_window(), abort_window);
...
fn submit_proposal_works() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(
			MolochV2::submit_proposal(Origin::signed(1), applicant, token_tribute, shares_requested, detail.clone()),
			Error::<Test>::NoCustodyFound
		);

		// deposit custody and resubmit
		assert_ok!(MolochV2::custody(Origin::signed(applicant), token_tribute));
		assert_ok!(MolochV2::submit_proposal(Origin::signed(1), applicant, token_tribute, shares_requested, detail));
